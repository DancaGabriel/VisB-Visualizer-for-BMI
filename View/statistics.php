...
if (isset($_POST['export_csv'])) {
    ...
    switch ($option) {
        case 'option1':
            ...
            if ($countries) {
                $filename = 'top_10_countries_increasing_obesity.csv';
                exportToCSV($countries, $filename);
            }
            break;
        case 'option2':
            $maxValue = $countryModel->getMaxValue();
            if ($maxValue && !empty($maxValue)) {
                $filename = 'top_10_countries_highest_obesity_risk.csv';
                exportToCSV($maxValue, $filename);
            }
            break;
        case 'option3':
            $topCountries = $countryModel->getTop10CountriesHighestAveragePreObesity();
            if ($topCountries) {
                $filename = 'top_10_countries_highest_pre_obesity_average.csv';
                exportToCSV($topCountries, $filename);
            }
            break;
        case 'option4':
            $sameValue = $countryModel->getSameValue();
            if ($sameValue && !empty($sameValue)) {
                $filename = 'same_value.csv';
                exportToCSV($sameValue, $filename);
            }
            break;
        default:
            break;
            ...
            <div class="container">
                ...
                <div class="export">
                    ...
                    <div class="action-buttons">
                        <button type="submit" name="export_csv" value="Export CSV">CSV</button>
                        <button type="submit" name="export_WebP" value="Export CSV">WebP</button>
                        <button type="submit" name="export_CSV" value="Export CSV">SVG</button>
                    </div>
                </div>
